#${micro}_summed_pop_cut in format <pop> <sum> where sum is the sum of the pvalues of SNPs with the strongest evidence of positive selection (for each micronutrient category, removing genes with secondary assignment)
#below for Fst, analogous for Relate
...
for y in micro: 
	newfile=[]
	for x in pops:
		dist = pd.read_csv("/home/ssd/jrees/significant/fst/sumstat/"+str(y)+"/{}_neutral_summed".format(x), header=None)
		mean = np.mean(dist[0])
		std = np.std(dist[0])
		file=pd.read_csv("/home/ssd/jrees/significant/fst/sumstat/"+str(y)+"_summed_pop", header=None, sep=" ")
		score = file.loc[file[0]=="{}".format(x)][1].values[0]
		prob = 1- norm(mean, std).cdf(score)
		newfile.append(prob) 
	file['Probability'] = newfile
	np.savetxt("/home/ssd/jrees/significant/fst/sumstat/"+str(y)+"_summed_pop", file, fmt = '%s %f %f',header="Population Sum Probability", comments="")
